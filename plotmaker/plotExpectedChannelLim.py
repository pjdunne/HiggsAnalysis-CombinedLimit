...
def makePlot():
  ...
  CMS_lumi.lumi_8TeV = "18.9-19.7 fb^{-1}"
  CMS_lumi.lumi_7TeV = "0-4.9 fb^{-1}"
  ...
  dummyHist = r.TH1D("dummy","",4,0,200.15)
  ...
  dir=('../exocombcards/125/')

  tfvbf = r.TFile(dir+'higgsCombineVBF.Asymptotic.mH125.root')
  tfggh = r.TFile(dir+'higgsCombineggH.Asymptotic.mH125.root')
  ...
  for i in range(treeggh.GetEntries()):
      treeggh.GetEntry(i)
      values.append([gghcenter, treeggh.limit])
  ...
  dummyHist.SetMaximum(2.6)#mg.GetYaxis().GetXmax())
  dummyHist.SetLineColor(0)
  dummyHist.SetStats(0)
  dummyHist.Draw("AXIS")
  xax = dummyHist.GetXaxis()
  vbf_bin_index = xax.FindBin(vbfcenter)
  ggh_bin_index = xax.FindBin(gghcenter)
  vh_bin_index = xax.FindBin(vhcenter)
  comb_bin_index = xax.FindBin(combcenter)
  xax.SetBinLabel(vbf_bin_index,"VBF-tagged" )
  xax.SetBinLabel(ggh_bin_index,"ggH-tagged" )
  xax.SetBinLabel(vh_bin_index,"VH-tagged" )
